...
|--------------------------|----------------------------------------|-----------------------------------------------------------------------|
| Port interfaces          | `IStore`, `IMutableStore`, `IInvalidatingStore`, `IIlingo` | `I`-prefix, reserved for **class-backed port interfaces** — a class `implements`/`extends` them (see commit `137dfd5 fix: rename interface Store -> IStore`). The `interface` keyword itself is used only for these and for declaration-merging targets (next row); everything else is a `type` — see [Interface vs. `type`](#interface-vs-type-declarations) below. `IIlingo` is the public type contract of the orchestrator; prefer it in type positions, reserve the concrete `Ilingo` class for construction. |
| Augmentable interfaces   | `ValidupCatalog`, `ValidupCatalogEntries` | Public declaration-merging targets — stay `interface` (a `type` alias can't be augmented), and keep their descriptive **unprefixed** name (it's the public `declare module` surface consumers extend, and they're data-shape contracts, not ports). |
...
**Default to `type`.** Use the `interface` keyword **only** when one of these two conditions holds; otherwise declare a `type` alias:
...
2. **It is a public declaration-merging target** — consumers reopen it via `declare module '…' { interface X { … } }`. A `type` alias **cannot** be augmented, so these *must* stay interfaces: `ValidupCatalog` / `ValidupCatalogEntries` in `@ilingo/validup`.

Everything else is a `type`: option bags, context objects, config shapes, result shapes, data-shape records — e.g. `StoreGetContext`, `StoreSetContext`, `MemoryStoreOptions`, `Config`/`ConfigInput`, `TemplateContext`, `TranslateIssueOptions`, `IssueTranslation`, `UseScopedCatalogResult`, `Lines`/`Namespaces`/`Locales`. A lone object shape that no class implements and no consumer augments gains nothing from `interface` and reads more consistently as a `type`. Converting such an `interface` to a `type` is **not** a breaking change — the exported name is unchanged.

### `I`-prefix scope

The `I`-prefix is reserved for category (1) — the **class-backed port interfaces only**: `IStore`, `IMutableStore`, `IInvalidatingStore`, `IIlingo`. It signals "this is the contract; the concrete class carries the bare name" (`IStore` ↔ `MemoryStore`, `IIlingo` ↔ `Ilingo`). Their runtime type guards keep the *unprefixed* verb form — `isMutableStore`, `isInvalidatingStore` — never `isIMutableStore`.

Category (2) augmentation targets are **not** prefixed (`ValidupCatalog`, not `IValidupCatalog`): the bare name is part of the public `declare module` surface, and they are data-shape contracts rather than ports — same rationale as the unprefixed `Lines`/`Namespaces`/`Locales`. `type` aliases are never `I`-prefixed.

No lint rule can express "class-backed or augmentable", so this is a **manual review convention** — apply it whenever adding or reviewing a declaration. When changing an `interface` ↔ class relationship (e.g. a new class starts implementing a former `type`), revisit both the keyword and the prefix in the same change.
