...
| Data-shape records       | `Lines`, `Namespaces`, `Locales` | The **normalized** catalog shapes (post `normalizeCatalog`) — `Locales` = `Record<locale, Namespaces>`, `Namespaces` = `Record<namespace, Lines>`, `Lines` = nested `string` / `PluralNode` leaves. Bare plural nouns; the `…Record` suffix was dropped when the names were freed. Authoring uses the descriptor-tree node types (`CatalogNode`, `LocaleNode`, `NamespaceNode`, `LinesNode`, `PluralNode`) instead. |
...
Category (2) augmentation targets are **not** prefixed (e.g. the former `ValidupCatalog`, not `IValidupCatalog`): the bare name is part of the public `declare module` surface, and they are data-shape contracts rather than ports — same rationale as the unprefixed `Lines`/`Namespaces`/`Locales`. There are no such targets in the tree today (`ValidupCatalog` was removed with type-safe keys), but the naming rule stands for any future one. `type` aliases are never `I`-prefixed.
