...
Category (2) augmentation targets are **not** prefixed (e.g. the former `ValidupCatalog`, not `IValidupCatalog`): the bare name is part of the public `declare module` surface, and they are data-shape contracts rather than ports — same rationale as the unprefixed `Translations`/`Namespaces`/`Locales`. There are no such targets in the tree today (`ValidupCatalog` was removed with type-safe keys), but the naming rule stands for any future one. `type` aliases are never `I`-prefixed.
