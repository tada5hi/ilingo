...
- [x] **Plural-leaf storage forms — decision: drop the bare structural form at the next major.** Soft deprecation landed first (warn-once at lookup time, behavior unchanged); hard removal is queued for the stable-major cut. Rationale: the structural form collides with sibling keys named after CLDR categories — the original "weird mix" complaint that motivated this work. Migration is a one-line wrap (`{ "@plural": ... }` for JSON, `definePlural({ ... })` for TS).
