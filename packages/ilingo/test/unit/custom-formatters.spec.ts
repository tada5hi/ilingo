    ...
    it('registerFormatter() exposes a custom modifier inside templates', async () => {
        const ilingo = new Ilingo({
            store: new MemoryStore({
                data: defineCatalog([
                    defineLocale('en', [
                        defineNamespace('app', [defineTranslations({ shout: 'Hi {{name, upper}}!' })]),
                    ]),
                ]),
            }),
        });
    ...
    it('Config.formatters can override a built-in formatter', async () => {
        const ilingo = new Ilingo({
            store: new MemoryStore({
                data: defineCatalog([
                    defineLocale('en', [
                        defineNamespace('app', [defineTranslations({ owe: 'You owe {{amount, number}}' })]),
                    ]),
                ]),
            }),
            formatters: {
    ...
    it('clone({ formatters }) applies the override formatters to the shared registry', async () => {
        ...
        const child = parent.clone({
            store: new MemoryStore({
                data: defineCatalog([
                    defineLocale('en', [
                        defineNamespace('app', [defineTranslations({ hi: 'hi {{name, upper}}' })]),
                    ]),
                ]),
            }),
            formatters: {
    ...
    it('clone() shares the formatter registry — custom formatters work in the child', async () => {
        ...
        const child = parent.clone({
            store: new MemoryStore({
                data: defineCatalog([
                    defineLocale('en', [
                        defineNamespace('scoped', [defineTranslations({ hi: 'hi {{name, upper}}' })]),
                    ]),
                ]),
            }),
        });
