...
Client-side applications, rely on manipulating the cache. This can be done during instance creation or afterward using the `setMany` method.
...
```typescript
import { setConfig, useIlingo } from 'ilingo/server';
